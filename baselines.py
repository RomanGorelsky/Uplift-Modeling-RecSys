...
class Recommender(object):
    ...
    def train(self, df, path, num_run, iter=100):
        pass
                ...
                for n in np.arange(len(df_train)):
                    ...
                    if current_iter % 10000000 == 0:
                        if not os.path.isdir(path):
                            os.makedirs(path)
                        with open(path + str(num_run) + "_" +"dlmf_weights.pkl", "wb") as f:
...
class DLMF_Mod(Recommender):
    ...
    def train(self, df, path, num_run, phi, iter = 100):
        # df_train = df.loc[df.loc[:, self.colname_outcome] > 0, :] # need only positive outcomes
        df_train = df.copy()
            ...
            if self.metric in ['AR_logi', 'AR_sig', 'AR_hinge']:
                for n in np.arange(len(df_train)):
                    ...
                    if current_iter % 10000000 == 0:
                        if not os.path.isdir(path):
                            os.makedirs(path)
                        with open(path + str(num_run) + "_" + "dlmf_mod_weights.pkl", "wb") as f:
                            pickle.dump(self.__dict__, f)
        ...
        while True:
            ...
            for n in np.arange(len(df_train)):
                ...
                if current_iter % 100000 == 0:
                        print(str(current_iter) + "/" + str(iter))
                if current_iter % 10000000 == 0:
                    with open(path + str(num_run) + "_" + "dlmf_dr_weights.pkl", "wb") as f:
                        pickle.dump(self.__dict__, f)
                        print("DLMF_DR weights saved.")
                if current_iter >= iter:
                        return err / iter
